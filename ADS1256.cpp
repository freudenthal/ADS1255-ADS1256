...
ADS1256::ADS1256(float clockspdMhz, float vref, uint8_t cspin, uint8_t readypin = 254, uint8_t resetpin = 254)
{
  ConnectionSettings = SPISettings(clockspdMhz * 1000000 / 4, MSBFIRST, SPI_MODE1);
  ReadyPin = readypin;
  ResetPin = resetpin;
  CSPin = cspin;
  if (ReadyPin != 254)
